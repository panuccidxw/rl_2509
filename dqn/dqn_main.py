...
'''
The instability of training in carpole_naive_dqn.py is caused by:
(P1) correlation between observations
(P2) chasing one's own tail
most reinforcement learning problem observations are time-series by nature, in which correlation is inherent.
while correlation is not conducive for convergence in learning, it carries valuable causal information.
Our solution:
(A) we keep the good part (causal information) by stacking sequential observation. See preprocessing in utils.py
(B) experience replay buffer to address (P1)
(C) target network to address (P2)
